					...
					.each(
							...
							function(i, item) {
								if (item[options.parentColumn] == parentNode[options.id]) {
									...
									tr.addClass('treegrid-parent-'
											+ parentNode[options.id]);
									$
											.each(
													options.columns,
													function(index, column) {
														var td = $('<td style="width:'
																+ column.width
																+ ';"></td>');
														if (column.field == "operation")
															td
																	.html('<div class="cbNew checkbox"><label style="color:green;"><input type="checkbox"  value="'+item[options.id] +'">启用</label></div>');
														else
															td
									...
									tbody.append(tr);
									// target.getChildNodes(data, item,
									// nowParentIndex, tbody)
